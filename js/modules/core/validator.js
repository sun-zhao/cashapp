define(function(require, exports, module) {
	...
	refresh = function(result, elem, mode, msg) {
		...
		if (li) {
			var inputType = $(li).attr('lang');
			var errorEL = false;
			if (inputType) {
				if (inputType == 'checkbox' || inputType == 'radio') {
					errorEL = $('p', li).first();
				} else {
					errorEL = $(elem).closest('.LiOut');
				}
				if (result) {
					$('.errorMsg', li).text('');
	...
	validate = function(el, mid) {
		...
		if (mode) {
			...
			var inputValue = $(el).val();
			if (blank) {
				var parentObj = $(el).parent('.LiOut');
				if (parentObj) {
					if ($(parentObj).hasClass('hideInmask')) {
						$(parentObj).removeClass('hideInmask');
					}
					if (mode['required']) {
						
						if (!$(parentObj).hasClass('error')) {
					...
					if (mode) {
						...
						if (obj) {
							...
							if (li) {
								var inputType = $(li).attr('lang');
								if (inputType) {
									if (inputType == 'varchar' || inputType == 'number' || inputType == 'amount' || inputType == 'text') {
